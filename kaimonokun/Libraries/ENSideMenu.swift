...
public extension UIViewController {
    ...
    // sidemenuと現在ViewControllerをカプセル化したENSideMenuProtocolを返却する
    ...
    // デフォルトのViewControllerをカプセル化したENSideMenuProtocolを返却する
    // ? internal
    internal func topMostController () -> ENSideMenuProtocol? {
        // 下記コードでルートのViewControllerを取得できる
...
public class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    // @param sourceView 最初に表示したいView
    // @param menuTableViewController メニューに対応するTableViewController
    // @param menuPosition 右か左
    ...
    // toggle, show, hideで使われるメニューの開閉を担当
    ...
    // スワイプ時に使われるアクション
    // @todo メニュー閉じるときはsourceViewではなくmenuのほうがタッチ認識される
